...
- **THEN** the system MUST store it with: unique ID, source layer (syndication, research, narrative, events, personal-graph), source URL, title, content, summary, structured metadata (key-value), timestamp of original publication, timestamp of ingestion
...
- **THEN** the structured metadata MUST capture layer-specific fields (e.g., author for research, event name for events, feed URL for syndication)

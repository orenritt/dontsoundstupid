...
Syndication ingestion is the always-on baseline layer for signal collection. It polls RSS/Atom feeds and uses scrape+diff as a fallback for sources without feeds. Feeds are derived from user profiles (peer orgs, impress list companies, industry publications).

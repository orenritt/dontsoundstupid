...
Narrative detection is the LLM-powered monitoring layer (Layer 3) in the signal generation pipeline. It watches large-scale coverage across news APIs, social trends, and search trends to detect emerging frames, language shifts, and trending narratives. Individual frames and term bursts are persisted; analysis snapshots are produced periodically for downstream consumption.

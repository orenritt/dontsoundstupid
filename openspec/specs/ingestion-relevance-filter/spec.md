# ingestion-relevance-filter Specification

## Purpose

The ingestion relevance filter applies a programmatic pre-filter to fetched news articles before they enter the signal store. Using the user's content universe (core topics and exclusions), it performs fast keyword matching to reject obviously irrelevant articles before they reach the more expensive LLM-based scoring agent gate.
